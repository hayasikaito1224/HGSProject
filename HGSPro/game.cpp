...
static const D3DXVECTOR3 LoseUIPos01 = { 800.0f,100.0f,0.0f };
static const D3DXVECTOR3 LoseUIPos02 = { 200.0f,100.0f,0.0f };
...
static const int FinishUIPop = 120 + (60*10);
static const int ResultPop = 120 + (60 * 10)+60;
...
void CGame::Update(void)
{
	...
	if (m_nUITimer >= ResultPop)
	{
		...
		if (nDifference01 > nDifference02)
		{
			if (!m_pWinUI)
			{
				m_pWinUI = CMove_UI::Create(WinUIPos01, WinUISize, 0, 10, CTexture::Test, CMove_UI::UI_Type::Type_Commendation);
			}
			if (!m_pLoseUI)
			{
				m_pLoseUI = CMove_UI::Create(LoseUIPos02, LoseUISize, 0, 10, CTexture::Test, CMove_UI::UI_Type::Type_Commendation);
			}
		}
		else if (nDifference01 < nDifference02)
		{
			if (!m_pWinUI)
			{
				m_pWinUI = CMove_UI::Create(WinUIPos02, WinUISize, 0, 10, CTexture::Test, CMove_UI::UI_Type::Type_Commendation);
			}
			if (!m_pLoseUI)
			{
				m_pLoseUI = CMove_UI::Create(LoseUIPos01, LoseUISize, 0, 10, CTexture::Test, CMove_UI::UI_Type::Type_Commendation);
			}
		}
